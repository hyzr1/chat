...
export async function operationalHealth() {
  ...
  const databaseHealthy = Object.values(quick || {}).some((value) => value === "ok");
  checks.push({ id: "database", category: "core", label: "Durable state", detail: databaseHealthy ? "SQLite integrity check passed; WAL-backed jobs are available." : "SQLite integrity check did not pass.", state: databaseHealthy ? "ready" : "unavailable", blocking: true });
  ...
  const accessControl = Boolean(productEnv("HYZR_CHAT_ACCESS_TOKEN", "VMX_ACCESS_TOKEN"));
  checks.push({ id: "access-control", category: "core", label: "LAN access control", detail: accessControl ? "Remote devices require a paired HttpOnly access cookie or bearer token." : "Developer mode is open on the local network. Set HYZR_CHAT_ACCESS_TOKEN before a customer pilot.", state: accessControl ? "ready" : "attention", blocking: false });
  ...
  checks.push({ id: "worker", category: "core", label: "Execution queue", detail: expiredLeases ? `${expiredLeases} expired worker lease${expiredLeases === 1 ? "" : "s"} awaiting recovery.` : `${activeJobs} active job${activeJobs === 1 ? "" : "s"}; no expired leases.`, state: expiredLeases ? "attention" : "ready", blocking: false });
  ...
  return {
    ...
    checkedAt: Date.now(),
    server: { version: "0.8.0", node: process.version, uptimeSeconds: Math.round(process.uptime()), platform: process.platform },
    storage: { databaseBytes, journalMode: "WAL" },
    checks,
  };
}
