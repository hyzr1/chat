...
import { CAPABILITY_LABELS, LOCAL_MODELS, LOCAL_TIER_DEFAULT, capabilityProfile, planUsageWeight, type TaskCapability } from "@/lib/local-models";
...
function SettingsModal({
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "models": {
        const coreIds = Object.keys(LOCAL_MODELS).filter((id) => !/[0-9]-(?:7|6)$|opus-3|sonnet-4-6/.test(id));
        const claudeIds = coreIds.filter((id) => LOCAL_MODELS[id].engine === "claude");
        const codexIds = coreIds.filter((id) => LOCAL_MODELS[id].engine === "codex");
        const heavy = new Set(["gpt-5.6-sol", "claude-fable", "gpt-5.5"]);
        const presets: Array<[string, string, string[]]> = [
          ["all", "All models", coreIds],
          ["save", "Save usage", coreIds.filter((id) => !heavy.has(id))],
          ["claude", "Claude only", claudeIds],
          ["chatgpt", "ChatGPT only", codexIds],
        ];
        const sameSet = (ids: string[]) => ids.length === modelPool.length && ids.every((id) => modelPool.includes(id));
        return <>
          <div className="models-settings-head"><div><strong>Allowed models</strong><span>{modelPool.length} enabled · routing always picks the best of these for each task</span></div></div>
          <div className="model-presets">{presets.map(([key, label, ids]) => <button key={key} className={`model-preset ${sameSet(ids) ? "on" : ""}`} onClick={() => onModelPool(ids)}>{label}</button>)}</div>
          <div className="model-settings-list">{Object.values(LOCAL_MODELS).map((model) => {
            const enabled = modelPool.includes(model.id);
            const profile = capabilityProfile(model.id);
            const w = planUsageWeight(model.id);
            return <div className={`model-setting-row ${enabled ? "enabled" : ""}`} key={model.id}>
              <span className="model-setting-mark"><BrandMark brand={brandFor(model.engine)} size={17} /></span>
              <div><strong>{model.label}</strong><span>{profile.bestFor}</span></div>
              <em className={`model-weight ${w >= 8 ? "high" : w >= 3 ? "mid" : "low"}`} title="Subscription usage per token vs. a mid model — higher drains your plan faster">×{w} usage</em>
              <em className="model-plan-tag">{model.plan}</em>
              <Toggle value={enabled} onChange={() => toggleAvailableModel(model.id)} disabled={enabled && modelPool.length === 1} />
            </div>;
          })}</div>
        </>;
      }
